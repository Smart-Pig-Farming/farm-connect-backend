...
async function debugScoring() {
  try {
    ...
    approvalEvents.forEach((event: any) => {
      console.log(`User: ${event.user_id}, Delta: ${event.delta}, PostID: ${event.ref_id}, Created: ${event.created_at}`);
    });
    ...
    if (unbalanced.length === 0) {
      ...
      unbalanced.forEach((item: any) => {
        console.log(`PostID: ${item.post_id}, User: ${item.user_id}, Approvals: ${item.approval_count}, Reversals: ${item.reversal_count}, Net Delta: ${item.net_delta}`);
      });
    }
